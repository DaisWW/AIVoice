...
登录 <http://127.0.0.1:18082/admin> 后，可创建成员账户和项目，并在“模型服务”中配置 ElevenLabs、MiniMax 或台词文本模型。台词文本模型支持 OpenAI Responses 和 Chat Completions 兼容接口；API Key 只保存在忽略版本控制的 `web/data/text-model-settings.json`，管理接口也只返回是否已配置，不返回密钥原文。

## AI 台词流程

1. 系统管理员进入“模型服务 → 台词文本模型”，填写兼容接口地址、模型 ID 和 API Key，并启用配置。地址可填写主机或带 `/v1` 的入口，模型 ID 允许使用供应商常见的 `provider/model` 形式。
2. 项目负责人或项目管理员在项目设置维护“项目总体提示词”，用于世界观、统一角色基调、语言风格、输出格式和禁用项等所有台本共用的规则。
3. 在台本详情维护“单台本提示词”，用于本台本的角色、场景、关系和特殊表达限制。它会叠加在项目总体提示词之后。
4. 点击“让 AI 完善提示词”只生成建议草稿；确认后点击“采用建议”，再手动保存。原提示词不会被自动覆盖。
5. 在台本详情的“AI 生成台词”填写本次要求和句数。服务会按“项目总体提示词 → 单台本提示词 → 本次要求”组合上下文，返回台词草稿。
6. 确认草稿后点击“加入台本编辑器”，检查台词和发音并保存行内容；之后再使用原有的声音模型流程生成音频。文本生成不会直接创建音频任务。

如果通过环境变量部署，也可使用 `VOICE_TEXT_MODEL_BASE_URL`、`VOICE_TEXT_MODEL_API_KEY` 和 `VOICE_TEXT_MODEL` 覆盖文件中的连接配置。生产环境应优先使用管理员页面保存配置，并限制 `web/data` 目录访问权限。
...
- 项目是唯一工作边界；创建者是负责人，项目管理员可维护资料、成员与成员权限。
- 台本只保存原始台词与发言顺序，可独立导入 `.txt`、`.md`、`.csv`、`.docx`。
- 声音支持 WAV、MP3、M4A、AAC、FLAC、OGG、OPUS，可上传多条真人录音并按参考语气分组。
- 每次生成形成独立批次，提交时可选择多个声音和模型；每个组合保存独立结果，便于对比。
- 每句生成 2 或 3 个候选；成员可共享试听和采用，全部句子完成审核后正式导出。
- 页面任意时刻只播放一段音频，新播放会自动暂停上一段。
- “全部下载”下载当前任务每段的默认克隆音频；“正式导出”下载每段已采用候选及 `UnityAudioManifest.json`。
- 成员只能访问自己加入的项目；同一项目成员共享声音、台本、生成批次和审核结果。系统管理员后台可查看全局状态并管理账户与模型服务。
